...
class Assets extends Command{
  Assets(){
    argParser.addFlag('generate',
        abbr: 'g',
        negatable: false,
        help: 'Generate assets class.');
    argParser.addFlag('listen',
        abbr: 'l',
        negatable: false,
        help: 'Listen to the assets folder.');
    argParser.addFlag('ignore',
        abbr: 'i',
        negatable: false,
        help: 'Ignore folder.');
  }
  List<String> _folders = ["assets"];
  final Settings settings = Settings();
  List<StreamSubscription<WatchEvent>> _activeWatchers = [];
  ...
  void _getFolders([String path = "assets"]) {
    ...
    if (directory.existsSync()) {
      ...
      print("❌ You don't have assets folder".red());
    }
  }
  ...
  _ignoreFolder() {
    ...
    settings.createFile({
      "ignored_assets_folders": newData
    });
  }
      ...
      if (directory.existsSync()) {
        directory.listSync().forEach((element) {
          ...
          if (element is File) {
            ...
            name = kk[0].toLowerCase()+kk.sublist(1).map((e) => e[0].toUpperCase()+e.substring(1).toLowerCase()).join();
...
//Get The path of the file that changed
  ...
  String get description => "Generate assets class and liten to the assets folder";
